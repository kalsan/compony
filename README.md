...
- GET will cause the Destroy component to ask if the resource should be destroyed, along with a button pointing to the DELETE verb.
...
- `on_destroyed_redirect_path` is evaluated as the second step of `on_destroyed_respond` and redirects to the resource's Show or Index page as described above. Overwrite this block in order to redirect to another component instead, while keeping the default flash provided by `on_destroyed_respond`.
